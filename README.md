...
After extracting the archive, point `$LLVM_SYS_221_PREFIX` to it:
```sh
export LLVM_SYS_221_PREFIX=</path/to/the/extracted/archive>/target-llvm/gnu/target-final
```
...
The `Makefile` provides a shortcut target to obtain a compatible LLVM build, using the provided [revive-llvm](crates/llvm-builder/README.md) utility. Once installed, point `$LLVM_SYS_221_PREFIX` to the installation afterwards:
...
To build the `resolc` Solidity frontend executable, make sure you have obtained a compatible LLVM build and did export the `LLVM_SYS_221_PREFIX` environment variable pointing to it (see [above](#LLVM)).
