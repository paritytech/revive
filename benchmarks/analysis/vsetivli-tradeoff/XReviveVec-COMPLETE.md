...
| CopyOverlapBug.CopyOverlapBug | 292 | 273 | 265 | 87 | 0.908× | 0.298× | 0.328× |
...
**Why it hurts — mechanism (decomposition + disassembly of the flat +9 modules).** After narrowing, the only i256 values left are the irreducible EVM storage/ABI words, and in these contracts they are overwhelmingly *constants or narrow values zext'd to 256* (`store i256 0`, counters widened to fill a 32-byte slot) — high limbs known-zero. The extension is pure overhead on that residual set, in three measured ways (identical across Events/Factorial/Fibonacci/MStore8/SubTypeValidation, all landing on exactly 87):

- **Wide-register marshalling the scalar path never pays** (`z_linear` = **+4**): each residual i256 crosses the scalar↔wide register-domain boundary before a `wst`/after a `wld`; a pure-scalar build stays in the scalar domain throughout.
- **Wide ops can't exploit known-zero limbs** (`z_memory` = 16): `store i256 0` / zext'd-narrow values have 3 zero limbs the scalar path elides, but `wst` transfers the full 256-bit word at `WIDE_MEMORY`=4 regardless.
- **Nothing to amortize against.** Disassembly: instruction count is flat (**308 scalar vs 305 wide**); scalar `sd`/`ld` halve (**32 → 16**) but are replaced ~1:1 by wide ops of equal-or-higher per-op cost — no long limb-chain of real 256-bit arithmetic is deleted, because on narrowed code there isn't one.

Net: **20 wide gas** (16 memory + 4 marshal) where the scalar build spent ~11 → the flat **+9**. The contrast that explains everything: on *un-narrowed* code the same wide op replaces a full 4-limb chain of *real* arithmetic (i256 `add` = 33 scalar gas vs 4 wide), so it wins (0.909×). **The extension's benefit is proportional to the wide _work_ it removes; narrowing removes that work, leaving only fixed marshalling overhead** — which is precisely why the two are substitutes.
...
The default path has **no** width *inference*: Yul's abstract machine has one type (the 256-bit word), so every **EVM stack/arithmetic value** is emitted `i256` and only LLVM's local `instcombine` narrows anything (the 99→84 above). The narrow integer types that *do* appear in the default IR are not inference: they are (a) machine plumbing — pointer/offset math and the byte-copy/keccak loops from linked runtime helpers, tagged `!tbaa` — and (b) a single hardcoded codegen emission for the `address` opcode (`load i160 @address_spill_buffer` → `bswap.i160` → **`zext i160 to i256`**). That i160 is definitional (an address is 20 bytes), and its *only* use is the `zext` that widens it straight back to i256 — the default path produces the narrow value and discards the narrowness one instruction later, which is exactly what "no inference" means: nothing narrow *propagates*. (The same i160→i256 `zext` appears byte-identical in the NewYork IR — the address is not something NewYork narrows either; its slot is 32 bytes.) NewYork adds a bidirectional dataflow pass — `crates/newyork/src/type_inference.rs` (2,054 lines) — that computes each value's minimum width to a fixed point and *keeps* it narrow across uses. That pass *is* the difference, and it narrows two ways.
...
|---|--:|--:|--:|
| Coinbase | 3 → 0 | 100% | 0.162× |
| BaseFee | 4 → 1 | 75% | 0.151× |
| GasLeft | 3 → 0 | 100% | 0.275× |
| Balance.BalanceReceiver | 1 → 0 | 100% | 0.259× |
| Value.ValueTester | 7 → 4 | 43% | 0.233× |
| Send | 9 → 7 | 22% | 0.232× |
| Context | 17 → 6 | 65% | 0.316× |
| **CallGas.Other** | **34 → 5** | **85%** | 0.356× |
| Selfdestruct.Tester | 3 → 2 | 33% | 0.459× |
| Baseline | 4 → 3 | 25% | 0.539× |

Every module narrows memory ops (never zero), so the mechanism is universal. But the *fraction* of memory ops narrowed only **moderately** predicts the gas win (Pearson r = −0.50): the total gas cut also comes from narrowing arithmetic/convert ops, and the small modules have too few memory ops for the fraction to be stable. CallGas is distinctive not because narrowing works differently there, but because it is by far the most **memory-dominated** module (34 i256 mem ops — the next is Context at 17), which is exactly why it was the lone module where the extension exceeded `ref` *before* narrowing (§6d, w 1.144×) and why narrowing pays off most in absolute terms there (653 → 193).
