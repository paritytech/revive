...
- **Loop-carried join**: a `for` loop joins the forward widths of every control edge reaching its phis — initializers and post yields for the loop variables, body yields and `continue` values for the post-region inputs, loop variables and `break` values for the outputs. Codegen builds the matching phi at that width, so a hash round written with `uint32` locals keeps its working variables 32-bit through the loop instead of being forced to `I256`.
...
- **Loop counter range narrowing**: Loop-carried variables take the join of every control edge reaching them, so masked working variables narrow, but a self-incrementing counter still saturates at `I256` because `add` widens by one level per round. Narrowing a bounded counter needs a range argument that the width lattice cannot express.
