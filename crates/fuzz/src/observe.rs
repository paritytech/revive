//! Execute compiled bytes against the EVM and PVM backends and capture
//! a common observation shape for downstream comparison.
...
/// What the differential comparison reads back from each backend.
///
/// `final_storage` is intentionally **not** part of the shape:
/// pallet-revive's storage can only be queried inside its
/// `ExtBuilder` closure, which has exited by the time the harness
/// returns. Storage divergence surfaces through subsequent action
/// return-data, as long as every template's `fn_0` body reads its
/// persisted state (enforced by inspection in
/// [`crate::templates`]; the `every_template_compiles` test does
...
/// Action calldata = `selector(fn_0(uint256)) || arg32`. Every
/// template exposes exactly one external function `fn_0`.
...
/// Deploy + replay actions against geth's `evm`, threading state
/// across calls via `Evm::from_genesis`.
...
/// Deploy + replay actions against `revive-runner`'s pallet-revive
/// sim. `differential: false` because we orchestrate the EVM side
/// ourselves.
