//! Warn when the installed `~/.cargo/bin/resolc` is older than any
//! revive workspace source — `compile_blob` spawns it as a
//! subprocess and silent-stale results would be misleading.

use std::path::{Path, PathBuf};
use std::time::SystemTime;

/// One-shot resolc-staleness warning. Safe to call from multiple
/// entry points; the underlying check is idempotent (logs at most
/// once per process via the `Once` guard).
...
fn check() {
    ...
    // crates/fuzz/.. = crates/, the source root we mtime-check.
...
/// Newest mtime under `crates/<member>/{src/**, Cargo.toml}`,
/// excluding `crates/fuzz` itself.
