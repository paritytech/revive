//! Solidity contract templates. Each renders a `pragma solidity ^0.8`
//! contract with the harness-required shape:
//! `constructor(uint256 seed)` + `fn_0(uint256 arg) external returns
//! (...)`. Templates must compile clean under solc 0.8.x, terminate
//! deterministically on every 256-bit input pair, and avoid block
//! context (`block.number`, `gas`, …) — that differs between geth's
//! `evm` and pallet-revive's sim and surfaces as benign divergence.

use std::fmt::Write;

use arbitrary::{Arbitrary, Unstructured};

/// Template family. Variant name is embedded in the contract name so
/// a divergence report identifies which family produced it.
#[derive(Debug, Clone, Copy)]
pub enum TemplateKind {
    /// `slot0 % arg` — original SREM probe (paritytech/revive#527).
    Srem,
    ArithChain,
    UncheckedArith,
    Mapping,
...
/// Render a contract; sub-choices (op selectors, etc.) drawn from `u`.
/// Name carries a random suffix to avoid blob-cache collisions across
/// concurrent fuzz cases.
...
/// `require` predicates; `a`, `b` are template-exposed locals.
...
fn render_srem(name: &str) -> String {
    ...
    let _ = writeln!(
        s,
        "    constructor(uint256 seed) {{ slot0 = int256(seed); }}"
    );
    let _ = writeln!(
        s,
        "    function fn_0(uint256 arg) external returns (int256) {{"
    );
...
fn render_loop_accum(u: &mut Unstructured<'_>, name: &str) -> arbitrary::Result<String> {
    ...
    // Cap at 31 iterations — keeps gas spend bounded.
...
fn render_bitwise(u: &mut Unstructured<'_>, name: &str) -> arbitrary::Result<String> {
    ...
    // Cap shift at 8 bits — larger shifts saturate to 0.
    let _ = writeln!(s, "        uint256 sh = arg & 0xff;");
    let _ = writeln!(
        s,
        "        uint256 r = (slot {op_a} arg) {op_b} (arg {shift} sh);"
    );
    let _ = writeln!(s, "        slot = r;");
    let _ = writeln!(s, "        return r;");
    let _ = writeln!(s, "    }}");
