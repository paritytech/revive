...
fn build_target(
    ...
    crate::utils::command(
        Command::new("emcmake")
            ...
            .env("LDFLAGS", "-s NO_INVOKE_RUN=1 -s EXIT_RUNTIME=1 -s ALLOW_MEMORY_GROWTH=1 -s EXPORTED_RUNTIME_METHODS=FS,callMain -s MODULARIZE=1 -s WASM_BIGINT=1 -s ALLOW_TABLE_GROWTH=1 -s NODEJS_CATCH_EXIT=0 -sDYNAMIC_EXECUTION=0")
            .arg("cmake")
            .args([
                "-S",
                source_directory.to_string_lossy().as_ref(),
