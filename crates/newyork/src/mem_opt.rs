...
impl FmpPropagation {
    ...
    /// pointer), a statically-resolved `mstore8` into the word, or an `ExternalCall`/`Create`.
    ///
    ...
    /// `fmp_writers` call case, which [`Self::statements_write_fmp`] misses. `constants` is the
    /// caller's constant-resolution scope at the region's entry, so offsets bound outside the
    /// region (and resolvable there) invalidate exactly as they would in straight-line code. Two
    /// uses:
