...
— direct `solc → EVM`, not the revive-yul roundtrip path the CLI
`revive-fuzz` binary defaults to. Direct solc gives pure
backend-vs-backend findings; the roundtrip path additionally
stresses revive-yul's parser/printer, which currently has known
divergences on dynamic-array access and similar shapes that drown
out backend findings under libFuzzer's fork-and-restart loop.

To exercise the revive-yul roundtrip path with coverage feedback
later, add a second fuzz target that calls `run_case` directly.
